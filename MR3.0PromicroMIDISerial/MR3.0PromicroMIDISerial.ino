...
// ============================================================================
// CONFIGURACIÓN DE HARDWARE (PRISMA MR3.0 DUAL MIDI 3 BANCOS)
// ============================================================================
...
const uint8_t NUM_BANKS = 3;  // <--- REDUCIDO A 3 BANCOS
const uint8_t NUM_PHYSICAL_BANK_LEDS = 5;
...
Adafruit_NeoPixel stripBanks(NUM_PHYSICAL_BANK_LEDS, PIN_BANK_LEDS, NEO_GRB + NEO_KHZ800);
...
struct Configuration {
  // Cambio el Magic Number para forzar un formateo de EEPROM al arrancar la primera vez con esta versión de 3 bancos
  char magic[2] = {'P', 'M'}; 
...
void initDefaultConfig() {
    gConfig.magic[0] = 'P'; gConfig.magic[1] = 'M'; gConfig.deviceId = 0x01;
    ...
    gConfig.analogs[0] = {0, 1023, 10, 11}; gConfig.analogs[1] = {0, 1023, 20, 11};
}

void loadConfigs() {
    EEPROM.get(0, gConfig);
    if (gConfig.magic[0] != 'P' || gConfig.magic[1] != 'M') {
        initDefaultConfig();
...
void sendSysExResponse(uint8_t cmd, const uint8_t* payload, size_t len) {
    ...
    serialmidi.sendSysEx(sysexBuffer, 5 + len);
}
...
public:
  void onSysExMessage(MIDI_Interface &midi_if, SysExMessage msg) override {
    if (msg.length >= 5 && msg.data[1] == SYSEX_MAN_ID && msg.data[2] == gConfig.deviceId) {
      ...
      if (cmd == CMD_GET_INFO) {
        const char* name = "PRISMA MR3.0 DUAL MIDI"; uint8_t nLen = strlen(name);
...
void refreshBankLEDs(bool force) {
  ...
  for(uint8_t i = 0; i < NUM_PHYSICAL_BANK_LEDS; i++) {
    // MAPEO PARA 3 BANCOS EN LA TIRA FÍSICA DE 5 LEDS (Indices 0, 2, 4 coinciden con los LEDs "1, 3 y 5")
    uint8_t targetPixel = 255;
    if (sel == 0) targetPixel = 0;
    else if (sel == 1) targetPixel = 2;
    else if (sel == 2) targetPixel = 4;

    if (i == targetPixel) { 
        stripBanks.setPixelColor(i, stripBanks.Color((gConfig.bankR*gConfig.bankBr)/255, (gConfig.bankG*gConfig.bankBr)/255, (gConfig.bankB*gConfig.bankBr)/255)); 
    }
...
void setup() {
    ...
    // Conectar Callbacks a las múltiples interfaces
