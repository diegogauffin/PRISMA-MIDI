...
const uint8_t CMD_REBOOT_BOOTLOADER = 0x0E;
...
// ============================================================================
// CLASE SMART POT (SOFT TAKEOVER) - RESOLUCIÓN Y MAPEO INTERNO
// ============================================================================

template <uint8_t NumBanks>
class CCSmartPotentiometer : public Updatable<> {
public:
  ...
  void update() override {
    ...
    if (analog.update()) {
      ...
      // Margen de seguridad para asegurar 0 y 127
      if (minV + 2 < 1023) minV += 2;
      if (maxV > 2) maxV -= 2;
      ...
      if (!isLocked[currentBank]) {
        if (currentMidiValue != lastValue[currentBank]) {
          // Dirección dinámica según UI (EEPROM) en tiempo real
          ...
          if (bankType == BankType::ChangeAddress) addr = addr + (int)currentBank; 
          
          midi.sendControlChange(addr, currentMidiValue);
  ...
  uint16_t getRawValue() const { return analog.getRawValue(); }

private:
  Bank<NumBanks> &bank;
  AH::FilteredAnalog<10, 3, uint16_t> analog; // Filtro a 3 para más respuesta rápida sin lag
  BankType bankType;
  uint8_t potIndex;
  ...
  void onSysExMessage(MIDI_Interface &midi_if, SysExMessage msg) override {
    if (msg.length >= 5 && msg.data[1] == SYSEX_MAN_ID && msg.data[2] == gConfig.deviceId) {
      ...
      if (cmd == CMD_GET_INFO) {
        ...
        if (idx < N_ANALOGS) {
          uint16_t cur = (idx == 0) ? potentiometer1.getRawValue() : potentiometer2.getRawValue();
      ...
      } else if (cmd == CMD_REBOOT_BOOTLOADER) {
        // Soft reset into Caterina Bootloader
...
void loop() {
    ...
    if (millis() - lastPotTime > 30) {
        uint16_t v0 = potentiometer1.getRawValue(); uint16_t v1 = potentiometer2.getRawValue();
