...
void initDefaultConfig() {
    ...
    for (uint8_t b = 0; b < NUM_BANKS; b++) {
        for (uint8_t i = 0; i < TOTAL_BUTTONS; i++) {
            gConfig.buttons[b][i] = {ButtonMode::CC, (uint8_t)(20+i), 7, 255, 255, 255, 255, 0, 127};
        }
    }
      ...
      if (cmd == CMD_GET_INFO) {
        ...
        for (uint8_t b = 0; b < NUM_BANKS; b++) { 
          ...
          for (uint8_t i = 0; i < TOTAL_BUTTONS; i++) {
            ...
            uint8_t d[] = { b, i, (uint8_t)cfg.mode, cfg.number, cfg.channel, (uint8_t)((cfg.r>>7)&1), (uint8_t)(cfg.r&127), (uint8_t)((cfg.g>>7)&1), (uint8_t)(cfg.g&127), (uint8_t)((cfg.b>>7)&1), (uint8_t)(cfg.b&127), (uint8_t)((cfg.brightness>>7)&1), (uint8_t)(cfg.brightness&127), cfg.flags, cfg.velocity };
        ...
        if (b < NUM_BANKS && idx < TOTAL_BUTTONS) {
            ...
            if (msg.length >= 20) cfg.velocity = msg.data[18];
