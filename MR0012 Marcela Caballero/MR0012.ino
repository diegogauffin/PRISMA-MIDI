...
// LEDs: {9, 10, 11, 12, 13}
IncrementDecrementSelectorLEDs<5> selector{bank, {7, 8}, {9, 10, 11, 12, 13}};
...
// Pulsador latched (CCButtonLatched) - Pulsador 4
...
constexpr analog_t minimumValue = 3500;
constexpr analog_t maximumValue = 6700;
...
void updateBankLeds() {
  ...
  // LED 0: asociado al pulsador del pin 5 (pulsador4) - Latched
  if (pulsador4.getState()) {
...
void loop() {
  ...
  static bool lastStates[4] = {false, false, false,
                               false}; // 4 pulsadores latched

  uint8_t currentBank = bank.getSelection();
  bool currentStates[4] = {pulsador1.getState(), pulsador2.getState(),
                           pulsador3.getState(), pulsador4.getState()};
